...
class Meal:
    ...
    def __post_init__(self):
        """Validates the Meal object attributes after initialization.

        Ensures that the price is positive and the difficulty is one of the allowed values: 'LOW', 'MED', 'HIGH'.
        """
        if self.price < 0:
...
def create_meal(meal: str, cuisine: str, price: float, difficulty: str) -> None:
    ...
    Args:
        ...
        difficulty: The difficulty level of preparing the meal (LOW, MED, or HIGH).

    Raises:
        ValueError: If the price is invalid or the difficulty level is incorrect.
        sqlite3.Error: If there is a database error, including integrity issues such as duplicate meal names.
    """
...
def clear_meals() -> None:
    """Clears all meals from the database by recreating the meals table.

    This operation deletes all the meals in the database.
...
def delete_meal(meal_id: int) -> None:
    ...
    Args:
        meal_id: The ID of the meal to be deleted.
...
def get_leaderboard(sort_by: str="wins") -> dict[str, Any]:
    """Retrieves the leaderboard of meals, sorted by the specified criteria.

    Args:
        sort_by: The criteria to sort the leaderboard by. It can be 'wins' or 'win_pct'.

    Returns:
        dict: A dictionary of meals sorted by the given criteria, with each meal's stats.
...
def get_meal_by_id(meal_id: int) -> Meal:
    ...
    Returns:
        Meal: The meal object corresponding to the given ID.
...
def get_meal_by_name(meal_name: str) -> Meal:
    ...
    Returns:
        Meal: The meal object corresponding to the given name.
...
def update_meal_stats(meal_id: int, result: str) -> None:
    ...
    Raises:
        ValueError: If the result is invalid or the meal has been deleted.
        sqlite3.Error: If there is a database error.
    """
    try:
        with get_db_connection() as conn:
